...
const { props } = usePage();
const { title, desc, vehicles, parking_rate, errors: pageErrors } = props;

const toast = useToast();

const form = useForm({
    vehicle_id: parking_rate ? parking_rate.vehicle_id : '',
    time_interval: parking_rate ? parking_rate.time_interval : '',
    cost: parking_rate ? parking_rate.cost : '',
});
...
function handleSubmit() {
    ...
    router.put(route('parking_rates.update', parking_rate.slug), form, {
        preserveScroll: true,
        preserveState: true,
        onSuccess: () => {
            toast.success('Setting parkir berhasil diubah!');
            form.reset();
