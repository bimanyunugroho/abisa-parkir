...
const { props } = usePage();
const { title, desc, parking_area, errors: pageErrors } = props;
const toast = useToast();

const form = useForm({
    name: parking_area.name,
    capacity: parking_area.capacity
});
...
function handleSubmit() {
    ...
    router.put(route('parking_areas.update', parking_area.slug), form, {
        preserveScroll: true,
        preserveState: true,
        onSuccess: () => {
            toast.success('Master area parkir berhasil diubah!');
            form.reset();
                    ...
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <form @submit.prevent="handleSubmit">
                            ...
                            <div class="mb-4">
                                ...
                                <TextNumeric id="capacity" v-model="form.capacity" class="mt-1 block w-full"
                                    placeholder="Enter capacity" :min="0" />
                                <p v-if="form.errors.capacity" class="mt-2 text-sm text-red-600 dark:text-red-400">
                                    {{ form.errors.capacity }}
                                </p>
                            </div>
                            <div class="flex items-center justify-end mt-4 space-x-2">
                                <SubmitButton :processing="isProcessing" :disabled="!form.isDirty"
                                    @click="handleSubmit" label="Update" />
                                <ActionLink :href="route('parking_areas.index')" label="Cancel" color="secondary" />
                            </div>
                        </form>
                    </div>
                </div>
